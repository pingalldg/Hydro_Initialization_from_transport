       ...
       profile >> neta >>  nx >> ny >> deta >> dx >> dy >> tau_ampt; 
        ...
        for (ix = 0; ix <= DATA->nx; ix++) {
            for (iy = 0; iy <= DATA->ny; iy++) {
               ...
               // temp_profile_utau[ix][iy] = sqrt(1+ux*ux+uy*uy+tau_ampt*tau_ampt*uetas*uetas);
               // temp_profile_ux[ix][iy] = ux;
               // temp_profile_uy[ix][iy] = uy;
               
               temp_profile_utau[ix][iy] =1.00;
               temp_profile_ux[ix][iy] = 0.00;
               temp_profile_uy[ix][iy] = 0.00;
        ...
        for(int ieta = 0; ieta < DATA->neta; ieta++)
        {
            ...
            for(ix = 0; ix <= DATA->nx; ix++)
            {
                for(iy = 0; iy<= DATA->ny; iy++)
                {
                    rhob = 0.0;
                    if(entropy_flag == 0)
                        epsilon = (temp_profile_ed[ix][iy]*eta_envelop_ed/hbarc); //pingal

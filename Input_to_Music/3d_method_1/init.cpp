     ...
     for(iy=0; iy<=DATA->ny; iy++)
       {
         ...
         if ( initializeEntropy==0 )
           {
             ...
             //rhob = DATA->rhoB0/epsilon0*epsilon; // /* in fm^-3 */ Pingal
        ...
        string dummy;
        int nx, ny, neta;
        double dx, dy, deta,tau_ampt;
        // read the information line
       // profile >> dummy >> dummy >> dummy >> dummy >> neta 
       //         >> dummy >> nx >> dummy >> ny 
       //         >> dummy >> deta >> dummy >> dx >> dummy >> dy; //pingal
       profile >> neta >>  nx >> ny >> deta >> dx >> dy >> tau_ampt; 
        ...
        double density, dummy1, dummy2, dummy3,dummy4;
        double ux, uy, uetas, utau; // pingal
        
        double*** temp_profile_ed = new double** [neta];
        double*** temp_profile_utau = new double** [neta];
        double*** temp_profile_ux = new double** [neta];
        double*** temp_profile_uy = new double** [neta];
        double*** temp_profile_uetas = new double** [neta];
        
        for (int i = 0; i < neta; i++){
        temp_profile_ed[i] =new double* [nx+1];
        temp_profile_utau[i] = new double* [nx+1];
        temp_profile_ux[i] = new double* [nx+1];
        temp_profile_uy[i] = new double* [nx+1];
        temp_profile_uetas[i] = new double* [nx+1];
        }       
        
        
        for (int j = 0; j < neta; j++){
        for (int i = 0; i < nx+1; i++) {
            temp_profile_ed[j][i] = new double [ny+1];
            temp_profile_utau[j][i] = new double [ny+1];
            temp_profile_ux[j][i] = new double [ny+1];
            temp_profile_uy[j][i] = new double [ny+1];
            temp_profile_uetas[j][i] = new double [ny+1];
        }
}
         int count=0;   
        //read the one slice
     for (int ieta = 0; ieta < DATA->neta; ieta++){
        for (ix = 0; ix <= DATA->nx; ix++) {
            for (iy = 0; iy <= DATA->ny; iy++) {
                ...
                profile >> dummy2 >> dummy3 >> dummy4 >> density >>utau>> ux >> uy >>uetas ; //pingal
                temp_profile_ed[ieta][ix][iy] = density;

              // temp_profile_utau[ieta][ix][iy] = 1./sqrt(1-ux*ux-uy*uy-uetas*uetas);
              // temp_profile_ux[ieta][ix][iy] = ux/sqrt(1-ux*ux-uy*uy-uetas*uetas);
              // temp_profile_uy[ieta][ix][iy] = uy/sqrt(1-ux*ux-uy*uy-uetas*uetas);
              // temp_profile_uetas[ieta][ix][iy] = uetas/sqrt(1-ux*ux-uy*uy-uetas*uetas);
               
               
               temp_profile_utau[ieta][ix][iy] = 1.00;
               temp_profile_ux[ieta][ix][iy] = 0.00;
               temp_profile_uy[ieta][ix][iy] = 0.00;
               temp_profile_uetas[ieta][ix][iy] = 0.00;
                ...
                if (ix == 0 && iy == 0 && ieta==0) {
                    ...
                    music_message.flush("info");
                       ...
                       cout << " hey! how you doing?" << count<< "   DATA->nx "<< DATA->nx <<endl;
        profile.close();
            ...
            for(ix = 0; ix <= DATA->nx; ix++)
            {
                for(iy = 0; iy<= DATA->ny; iy++)
                {
                   count=count+1;
                    ...
                    rhob = 0.0;
                    if(entropy_flag == 0)
                        epsilon = temp_profile_ed[ieta][ix][iy]/hbarc;
                                   //(temp_profile_ed[ix][iy]*eta_envelop_ed/hbarc); //pingal
                                  // *DATA->sFactor/hbarc);  // 1/fm^4
                    else
                    {
                        double local_sd = (temp_profile_ed[ieta][ix][iy]
                    ...
                    /* for HIC */
                    u[0] = (*arena)[ix][iy][ieta].u[0][0] = temp_profile_utau[ieta][ix][iy];
                    u[1] = (*arena)[ix][iy][ieta].u[0][1] = temp_profile_ux[ieta][ix][iy];
                    u[2] = (*arena)[ix][iy][ieta].u[0][2] = temp_profile_uy[ieta][ix][iy];
                    u[3] = (*arena)[ix][iy][ieta].u[0][3] = temp_profile_uetas[ieta][ix][iy];
                   
                    (*arena)[ix][iy][ieta].prev_u[0][0] = temp_profile_utau[ieta][ix][iy];
                    (*arena)[ix][iy][ieta].prev_u[0][1] = temp_profile_ux[ieta][ix][iy];
                    (*arena)[ix][iy][ieta].prev_u[0][2] = temp_profile_uy[ieta][ix][iy];
                    (*arena)[ix][iy][ieta].prev_u[0][3] = temp_profile_uetas[ieta][ix][iy];
        ...
        // clean up
                           cout << " hey! how you doing?" << count<< endl;
        for (int j = 0; j < neta; j++){
        for(int i = 0; i < nx+1; i++)
        {
            delete[] temp_profile_ed[j][i];
            delete[] temp_profile_utau[j][i];
            delete[] temp_profile_ux[j][i];
            delete[] temp_profile_uy[j][i];
            delete[] temp_profile_uetas[j][i];
        }
        }
        for (int j = 0; j < neta; j++){
        delete[] temp_profile_ed[j];
        delete[] temp_profile_utau[j];
        delete[] temp_profile_ux[j];
        delete[] temp_profile_uy[j];
        delete[] temp_profile_uetas[j];
        }
        delete[] temp_profile_ed;
        delete[] temp_profile_utau;
        delete[] temp_profile_ux;
        delete[] temp_profile_uy;
        delete[] temp_profile_uetas;       

...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine ampttohydro1(k,dx,tau_ampt,sigma,netas,netas_print,detas,etas_min,etas_wind,ks_0)
...
integer,intent (in)::k
real*8::posx,posy,sigma,dx,E,etas,a,tau,etas_min,detas,etas_wind,ks_0
real*8::xx,yy,zz,pxx,pyy,pzz,massxx,ft5,ftnd,taut,pressure,energy,min_etas,max_etas,real1
real*8::ppp(np),eee(np),y2(np)
real*8,dimension(n,n,0:k-1,0:k-1)::Tmunu,pimunu
real*8,dimension(n,0:k-1,0:k-1)::umu
real*8,dimension(0:k-1,0:k-1)::en,pr
real*8::p(n)
real*8::gmunu(n,n),gUmunu(n,n)!!!!!gmunu lower,gUmunu upper metric tensor!!!!!
integer::steps,count,int1,int2,int3,int4,int5,int6,int7,int8
real*8::tau_ampt
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! etas circle!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
do ietas=1,netas
      min_etas=etas_min+(ietas-1)*detas
      max_etas=min_etas+etas_wind
      write(*,*)min_etas,max_etas
	...
	do xi=xbegin,xend-1
	 do yi=ybegin,yend-1
		 ...
		 do o=l,4
                  ...
                  call distribute1(Tmunu,xi,yi,posx,posy,l,o,p,taut,k,dx,etas_wind,sigma,ks_0)
...
do xi=0,k-1
 do yi=0,k-1
 write(220,*)(xi-int(k/2.))*dx,(yi-int(k/2.))*dx,(min_etas+max_etas)/2.0,en(xi,yi), umu(1,xi,yi),umu(2,xi,yi),&
...
end subroutine ampttohydro1
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!END!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!END!!!!!!END!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!END!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine distribute1(Tmunu,xi,yi,posx,posy,l,m,p,taut,k,dx,etas_wind,sigma,ks_0)
integer::xi,yi,l,m
integer,parameter::n=4
integer,intent (in)::k
real*8::x,y,posx,posy,taut,dx,pi=2*asin(1.d0),etas_wind,sigma,ks_0
real*8,dimension(n,n,0:k-1,0:k-1)::Tmunu
real*8::p(n)
f(x,y)=ks_0/(2.*pi*(sigma**2)*taut*etas_wind)*(p(l)*p(m)/p(1))*exp(-(x**2+y**2)/(2*sigma**2))
x=(xi-int(k/2.))*dx
y=(yi-int(k/2.))*dx        

Tmunu(l,m,xi,yi)=Tmunu(l,m,xi,yi)+f(x-posx,y-posy)

end subroutine distribute1
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
